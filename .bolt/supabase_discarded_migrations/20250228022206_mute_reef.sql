...
-- Drop case type and status constraints
DO $$ 
BEGIN
  ...
  IF EXISTS (
    ...
    AND constraint_name = 'cases_type_check'
  ) THEN
  ...
  IF EXISTS (
    ...
    FROM information_schema.table_constraints 
    WHERE table_name = 'cases' 
    AND constraint_name = 'cases_status_check'
  ) THEN
    ALTER TABLE cases DROP CONSTRAINT cases_status_check;
  END IF;

  -- Drop sub_type check constraint if it exists
  IF EXISTS (
    SELECT 1 
    FROM information_schema.table_constraints 
    WHERE table_name = 'cases' 
    AND constraint_name = 'cases_sub_type_check'
  ) THEN
    ALTER TABLE cases DROP CONSTRAINT cases_sub_type_check;
  END IF;
END $$;