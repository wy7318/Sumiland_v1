...
import {
  Save, X, AlertCircle, Mail, Phone, Building2, User, Search, Calendar, Users,
  ArrowLeft, Edit, UserCheck, MapPin, Bookmark, FileText, MessageSquare, Briefcase,
  Globe, CheckCircle, ChevronDown, ChevronRight, Home,
  UserCircle, Settings
} from 'lucide-react';
import { supabase } from '../../lib/supabase';
import { useAuth } from '../../contexts/AuthContext';
...
export function CustomerForm() {
  ...
  const renderSectionHeader = (
    ...
    <button
      ...
      className="flex items-center justify-between w-full px-6 py-4 text-left bg-gray-50 border border-gray-200 hover:bg-gray-100 transition-colors"
    >
      <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex space-x-3">
                <button
                  type="button"
                  onClick={() => navigate('/admin/customers')}
                  className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg text-gray-600 bg-white border border-gray-300 hover:bg-gray-50 transition-colors"
                >
                  <X className="w-4 h-4 mr-2" />
                  Cancel
                </button>
                <button
                  ...
                  disabled={loading}
                  className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg text-white bg-cyan-600 hover:bg-cyan-700 disabled:bg-cyan-400 transition-colors shadow-sm"
                >
                  <Save className="w-4 h-4 mr-2" />
                  {loading ? 'Saving...' : 'Save Customer'}
                </button>
          ...
          {/* <div className="bg-white rounded-xl p-4 border border-gray-200">
            <div className="flex items-center space-x-3">
              ...
              <select
                value={formData.organization_id}
                onChange={(e) => setFormData(prev => ({ ...prev, organization_id: e.target.value }))}
                className="ml-auto px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-200 focus:border-cyan-500 outline-none"
                required
              >
          ...
          <div className="space-y-0">
            ...
            <AnimatePresence>
              {expandedSections.basic && (
                ...
                >
                  <div className="bg-white border border-gray-200 border-t-0 p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <input
                          ...
                          value={formData.first_name}
                          onChange={(e) => setFormData(prev => ({ ...prev, first_name: e.target.value }))}
                          className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-cyan-200 focus:border-cyan-500 outline-none transition-colors"
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            value={formData.email}
                            onChange={(e) => setFormData(prev => ({ ...prev, email: e.target.value }))}
                            className="w-full pl-10 pr-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-cyan-200 focus:border-cyan-500 outline-none transition-colors"
                        ...
                        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <div className="text-sm text-gray-500">Type: {selectedVendor.type}</div>
                            </div>
                          </div>
                          <button
                            type="button"
                          ...
                          <AnimatePresence>
                            {showVendorDropdown && vendorSearch && (
                              <motion.div
                                ...
                                animate={{ opacity: 1, y: 0 }}
                                exit={{ opacity: 0, y: -10 }}
                                className="absolute z-50 w-full mt-2 bg-white rounded-lg shadow-xl border border-gray-200 max-h-60 overflow-auto"
                              >
                                {filteredVendors.length > 0 ? (
                                  <ul className="py-2">
                                    {filteredVendors.map(vendor => (
                                      ...
                                      >
                                        ...
                                        <div className="text-sm text-gray-500">Type: {vendor.type}</div>
                                      </li>
                                    ))}
                                  </ul>
                                ) : (
