...
-- Create trigger function to handle quote status changes with better validation
...
BEGIN
  ...
  IF NEW.status = 'Approved' THEN
    ...
    -- Check if status can be changed to Approved
...
'Creates a new order from a quote with optimized performance, error handling, and status validation';

COMMENT ON FUNCTION handle_quote_status_change() IS 
'Handles quote status changes with improved validation to prevent duplicate approvals and invalid transitions';