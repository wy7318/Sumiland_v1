...
Performs all validations in a single function with proper row locking.';

COMMENT ON FUNCTION handle_quote_status_change() IS 
'Handles quote status changes by delegating to create_order_from_quote function.
Keeps trigger logic simple to avoid race conditions.';