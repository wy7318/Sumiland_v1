...
-- Create simple trigger function that just calls create_order_from_quote
CREATE OR REPLACE FUNCTION handle_quote_status_change()
RETURNS TRIGGER AS $$
BEGIN
  -- Only proceed if status is being changed to Approved
  IF NEW.status = 'Approved' AND OLD.status != 'Approved' THEN
    -- Let create_order_from_quote handle all validation
    BEGIN
...
Handles all validation and locking in a single function.';

COMMENT ON FUNCTION handle_quote_status_change() IS 
'Simple trigger function that delegates all logic to create_order_from_quote.
