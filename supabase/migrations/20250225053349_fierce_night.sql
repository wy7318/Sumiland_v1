...
EXCEPTION
  WHEN unique_violation THEN
    -- Handle the case where another transaction created the order
    RAISE EXCEPTION 'Order already exists for this quote';
  WHEN OTHERS THEN
...
-- Add unique constraint to prevent duplicate orders for the same quote
ALTER TABLE order_hdr
ADD CONSTRAINT unique_quote_order UNIQUE (quote_id);
...
Checks for existing orders before and after acquiring locks.
...
COMMENT ON CONSTRAINT unique_quote_order ON order_hdr IS 
'Ensures that only one order can be created for each quote.';