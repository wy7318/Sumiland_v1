...
DO $$ 
BEGIN
  -- Create custom_field_type enum if it doesn't exist
  ...
  -- Create custom_field_entity enum if it doesn't exist
...
-- Create table for custom field definitions if it doesn't exist
...
-- Create table for custom field values if it doesn't exist
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_custom_fields_org') THEN
    CREATE INDEX idx_custom_fields_org ON custom_fields(organization_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_custom_fields_entity_type') THEN
    CREATE INDEX idx_custom_fields_entity_type ON custom_fields(entity_type);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_custom_fields_status') THEN
    CREATE INDEX idx_custom_fields_status ON custom_fields(status);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_custom_field_values_org') THEN
    CREATE INDEX idx_custom_field_values_org ON custom_field_values(organization_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_custom_field_values_field') THEN
    CREATE INDEX idx_custom_field_values_field ON custom_field_values(field_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_custom_field_values_entity') THEN
    CREATE INDEX idx_custom_field_values_entity ON custom_field_values(entity_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_custom_field_values_value') THEN
...
DROP POLICY IF EXISTS "Custom fields are viewable by organization members" ON custom_fields;
DROP POLICY IF EXISTS "Custom fields can be managed by organization admins" ON custom_fields;
DROP POLICY IF EXISTS "Custom field values are viewable by organization members" ON custom_field_values;
DROP POLICY IF EXISTS "Custom field values can be managed by organization members" ON custom_field_values;
...
DROP TRIGGER IF EXISTS validate_custom_field_value_trigger ON custom_field_values;
